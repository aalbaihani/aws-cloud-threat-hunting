-- ============================================================
-- Cloud threat detections over CloudTrail via Athena
-- Table: cloudtrail_logs | Region: ap-southeast-2
-- Activity fingerprint window: 2026-09-13T14:00:00Z onward
-- ============================================================

-- D1 — Defense evasion: CloudTrail logging disabled (T1562.001)
-- StopLogging is one of the highest-fidelity cloud signals.
SELECT eventtime, eventname, useridentity.arn AS actor, sourceipaddress
FROM cloudtrail_logs
WHERE eventname IN ('StopLogging','StartLogging')
ORDER BY eventtime;

-- D2 — Rogue admin account: user created then granted AdministratorAccess (T1136/T1098)
SELECT eventtime, eventname, useridentity.arn AS actor, sourceipaddress
FROM cloudtrail_logs
WHERE eventname IN ('CreateUser','AttachUserPolicy','DeleteUser')
ORDER BY eventtime;

-- D3 — Persistence: access key created (T1098)
SELECT eventtime, eventname, useridentity.arn AS actor, sourceipaddress
FROM cloudtrail_logs
WHERE eventname = 'CreateAccessKey'
ORDER BY eventtime;

-- D4 — S3 defense weakening: public-access block removed (T1562)
SELECT eventtime, eventname, useridentity.arn AS actor, sourceipaddress
FROM cloudtrail_logs
WHERE eventname IN ('DeletePublicAccessBlock','PutBucketPublicAccessBlock')
ORDER BY eventtime;

-- D5 — Failed privilege use: denied/errored API calls (T1078)
SELECT eventtime, eventname, errorcode, sourceipaddress
FROM cloudtrail_logs
WHERE errorcode IS NOT NULL
  AND eventname IN ('AssumeRole','GetSessionToken','CreateUser','AttachUserPolicy')
ORDER BY eventtime;

-- D6 — Discovery burst: high volume of IAM enumeration in a short window (T1087)
SELECT date_trunc('minute', from_iso8601_timestamp(eventtime)) AS minute,
       count(*) AS iam_read_calls
FROM cloudtrail_logs
WHERE eventsource = 'iam.amazonaws.com'
  AND (eventname LIKE 'List%' OR eventname LIKE 'Get%')
GROUP BY date_trunc('minute', from_iso8601_timestamp(eventtime))
HAVING count(*) >= 3
ORDER BY minute;